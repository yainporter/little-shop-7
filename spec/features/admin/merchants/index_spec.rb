  ...
  describe 'As an Admin' do
    ...
    describe "User Story 30 - Admin Merchants Top 5 Merchants by Revenue" do
      before do
        ...
        #revenue calc notes
          # invoices with >= 1 successful transaction are counted towards total revenue
          # invoice revenue = sum of all invoice items' revenue
          # invoice item revenue = invoice_item.unit_price * quantity
              # unit_price
                # sold at -> invoice_item
                # current sale price -> item

          # total merchant revenue = sum (invoice revenues)
          # invoice revenue = sum (invoice_item.unit_price * quantity)
      end
